...
######################################################################################
# -------------------------------------
# Tuning the hyperparameter- Threshold
# -------------------------------------
######################################################################################
...
def plot_threshold_diagnostics(
    ...
    4. Pareto Front (MSE vs Sparsity)
    ...
    # --- (D) Pareto Front (MSE vs Sparsity) ---
    fig, ax = plt.subplots(figsize=(7, 5), dpi=dpi)
    sc = ax.scatter(
        nonzero_terms,
        mses,
...
######################################################################################
# -----------------------------------------------------
# Populate the results of the previous step in a table 
...
######################################################################################
# --------------------------------------------------------------------------------------
# Fit the model for the best threshold and then, Populate the results for each equation. 
...
def evaluate_model_per_equation(
    ...
    results_k   = results.sort_values('Active Terms', ascending=True)
    ...
    formatter = {
        ...
        'AIC': "{:.4f}".format,
        'BIC': "{:.4f}".format,
        'Active Terms': "{:d}".format
    }

    # 7) Highlight best (tie-aware)
    ...
    display_with_highlighted_row("Models sorted by lowest Active Terms", results, 'Active Terms', True, formatter, best_type="min")
    ...
    fig = px.scatter(
        ...
        title=f"Relative error for each model (Threshold={best_threshold})",
